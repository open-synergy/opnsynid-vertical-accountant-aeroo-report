...
class Parser(report_sxw.rml_parse):
    ...
    def _get_category(self, index):
        obj_cat = self.pool.get("accountant.general_audit_fraud_inquiry")
        header = index
        cat_ids = obj_cat.search(
            self.cr, self.uid, [("index_a2308_id.id", "=", header)], order="category_id"
        )
        no = 0
        cek_cat = "x"
    ...
    def _get_line(self, index, categ):
        self.line_data = []
        obj_line = self.pool.get("accountant.general_audit_fraud_inquiry")
        header = index
        line_ids = obj_line.search(
            self.cr,
            self.uid,
            [("index_a2308_id.id", "=", header), ("category_id.id", "=", categ.id)],
        )
        nomor = 0
        for line in obj_line.browse(self.cr, self.uid, line_ids).sorted(
            key=lambda s: s.fraud_inquiry_id.sequence
        ):
            nomor += 1
            res = {
                "nomor": nomor,
                "uraian": line.fraud_inquiry_id.name,
                "tcwg": line.tcwg_respon,
                "management": line.management_respon,
                "other": line.other_respon,
